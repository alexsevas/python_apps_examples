...
'''
A real-time object detection and tracking UI built with Ultralytics YOLO11 and OpenCV, designed for interactive demos
and seamless integration of tracking overlays./Пользовательский интерфейс для обнаружения и отслеживания объектов в
реальном времени, созданный с использованием Ultralytics YOLO11 и OpenCV, предназначен для интерактивных демонстраций
и плавной интеграции наложений отслеживания.

Whether you're just getting started with object tracking or looking to enhance it with additional features, this project
provides a solid foundation./Независимо от того, начинаете ли вы заниматься отслеживанием объектов или хотите
усовершенствовать его с помощью дополнительных функций, этот проект обеспечивает прочную основу.

Features:/Особенности:
- Real-time object detection and visual tracking/ Обнаружение объектов в режиме реального времени и визуальное отслеживание
- Click-to-track any detected object/ Нажмите, чтобы отследить любой обнаруженный объект
- Scope lines and bold bounding boxes for active tracking/ Линии обзора и жирные рамки для активного отслеживания
- Dashed boxes for passive (non-tracked) objects/ Пунктирные рамки для пассивных (не отслеживаемых) объектов
- Live terminal output: object ID, label, confidence, and center coordinates/ Оперативный вывод с терминала: идентификатор
объекта, метка, достоверность и координаты центра
- Adjustable object tracking algorithms (ByteTrack, BoT-SORT)/ Настраиваемые алгоритмы отслеживания объектов (байтрекинг, бот-СОРТИРОВКА)

Supports:/ Поддерживает:
- PyTorch .pt models (for GPU devices like NVIDIA Jetson or CUDA-enabled desktops)/
Модели PyTorch .pt (для устройств с графическим процессором, таких как NVIDIA Jetson или настольные компьютеры с поддержкой CUDA)
- NCNN .param + .bin models (for CPU-only devices like Raspberry Pi or ARM boards)/
Модели NCNN .param + .bin (для устройств только с процессором, таких как платы Raspberry Pi или ARM)
'''

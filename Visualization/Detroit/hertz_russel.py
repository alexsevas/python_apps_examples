...
# pip install detroit polars
# detroit is Python implementation of d3js
...
def bv2rgb(bv):
    ...
    result = "".join(map(lambda t: hex(round(t * 255))[2:].zfill(2), (a, b, c)))
    return f"#{result}"
...
(
    ...
    .call(
        d3.axis_left(
            d3.scale_log(list(map(lambda m: 10 ** (4.83 - m), y.get_domain())), y.get_range())
        )
    )
)
...
with open("hertz_russel.svg", "w", encoding="utf-8") as file:
    file.write(str(svg))